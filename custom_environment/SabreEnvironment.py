...
class raw_env(AECEnv):
    ...
    def observation_space(self, agent):
        ...
        if agent == 'cp':
            # Define client info dictionary
            client_info_dict = {
                f'client{i}': Dict(
                    {
                        'position': Tuple([Discrete(self.x), Discrete(self.y)]),
                        'fetchingOrigin': Tuple([Discrete(self.x), Discrete(self.y)])
                    }
            ...
            cdn_info_dict = {
                f'cdn{i}': Dict(
                    {
                        'pricing': Box(low=np.array([0]), high=np.array([10]), dtype=np.float32),  # Replace with the appropriate Gym space for float
                        'edgeServer': Tuple([Tuple([Discrete(self.x), Discrete(self.y)]) for _ in range(self.maxEdgeServers)]),
                        'soldContigent': Discrete(10000)  # Replace with the appropriate Gym space for int
                    }
            ...
            cp_obs_space = Dict(
                {
                    'clientsInfo': Dict(client_info_dict),
                    'cdnsInfo': Dict(cdn_info_dict)
                }
            )
        
            self._observation_space_cache[agent] = cp_obs_space
            return cp_obs_space
        elif agent == 'cdn0' or agent == 'cdn1':
    ...
    def observe(self, agent):
        ...
        if agent == 'cp':
            ...
            # All client data for CP: ClientId, ClientPosition, ClientFetchingOrigin, ClientQoE
            clientsInfo = {}

            for client in self.clients.values():
                clientPosition = Tuple([Discrete(client.position[0]), Discrete(client.position[1])])
                clientFetchOrigin = Tuple([Discrete(client.fetchEdgeServer[0]), Discrete(client.fetchEdgeServer[1])])#client.fetchEdgeServer
                clientsInfo[client.id] = Dict({'position': clientPosition, 'fetchingOrigin': clientFetchOrigin})
                
            # All cdn data for CP: CdnId, CdnPricing, CdnEdgeServers, CdnSoldContigent
            cdnsInfo = {}
            for cdn in self.cdns.values():
                cdnDict = {'pricing': cdn.pricingFactor, 'edgeServer': Tuple([Discrete(cdn.edgeServers[0][0]), Discrete(cdn.edgeServers[0][1])]), 'soldContigent': cdn.soldContigent}
                cdnsInfo[cdn.id] = cdnDict
        ...
        # For CDN
            ...
            clientPositions = []
            for client in self.clients.values():
                clientPositions += client.position
